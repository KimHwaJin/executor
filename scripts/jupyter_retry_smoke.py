...
async def _wait(
    client: Client, execution_id: str, terminal: set[str]
) -> dict[str, Any]:
    for _ in range(200):
    ...
    async with Client("http://127.0.0.1:8000/mcp") as client:
        ...
        for attempt in attempts:
            ...
            if detail_result.is_error or steps_result.is_error:
                raise RuntimeError(
                    "Attempt detail or Step history lookup failed."
                )
        ...
        if (
            ...
            or attempt_details[0]["recovery"]["retry_strategy"]
            != "FROM_FAILED_STEP"
            or attempt_details[1]["runtime"]["session_id"]
            != original_runtime_session
            or attempt_details[1]["runtime"]["target_id"]
            != original_runtime_target
            ...
            raise RuntimeError(
                f"Attempt Step history is incomplete: {attempts}"
            )
        event_types = {
            event["event_type"]
        ...
        if not required_events.issubset(event_types):
            raise RuntimeError(
                f"Execution event history is incomplete: {event_types}"
            )
        retry_artifacts = [
            artifact
        ...
        if [artifact["status"] for artifact in retry_artifacts] != [
            "INCOMPLETE",
            "AVAILABLE",
        ] or retry_artifacts[0]["produced_by"][
            "execution_attempt_id"
        ] == retry_artifacts[1]["produced_by"]["execution_attempt_id"]:
            raise RuntimeError(
        ...
        print(
            "same_kernel:",
            attempt_details[1]["runtime"]["session_id"]
            == original_runtime_session,
        )
        print("attempts_in_trace:", len(attempts))
        print("event_count:", len(events_result.structured_content["items"]))
        print(
            "retry_artifact_statuses:",
            [item["status"] for item in retry_artifacts],
        )
