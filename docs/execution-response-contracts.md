...
Execution detail groups information by responsibility: `context`, `source`, `runtime`, `workspace`,
...
Step detail exposes Executor-owned `step_id`, `sequence`, `code_hash`, `step_timeout_seconds`,
`lineage`, `result`, lifecycle timestamps, and audit fields. Operation detail exposes its sequence
range, `operation_timeout_seconds`, metadata, immutable source reference, result, and audit fields.
