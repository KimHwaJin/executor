...
Attempt의 오류는 보존된다. 현재 필수 완료 정책에서는 코드가 성공했어도 노트북
저장·필수 아티팩트 등록·최종 런타임 해제에 실패하면 Execution은
`FAILED / COMPLETION_FAILED / NOT_RETRYABLE`이다. 성공한 Step과 출력 파일은
보존되며, 코드 재실행은 복구 방법이 아니다. `COMPLETION_FAILED` 진단의 phase와
앞서 기록된 원인 진단을 함께 확인한다. 일시 오류가 복구되었거나 이전 MULTI
Operation이 실패했던 이력은 최종 성공 후에도 남을 수 있다.
자세한 기준은 [필수 결과 완료 정책](../docs/required-result-completion.md)을 참고한다.
...
새 head는 `0003`다. 새 애플리케이션보다 마이그레이션을 먼저 적용한다.
