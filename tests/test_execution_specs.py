...
from executor_service.execution_specs import ExecutionSpec, ExecutionSpecResolver
...
async def test_inline_and_path_resolve_to_the_same_step_content(tmp_path: Path) -> None:
    ...
    path = _spec({"type": "PATH", "path": "steps/step-0.py", "sha256": checksum})
...
async def test_path_rejects_hash_mismatch_and_pv_escape(tmp_path: Path) -> None:
    ...
    with pytest.raises(InvalidExecutionSpecError, match="SHA-256"):
        await resolver.resolve(_spec({"type": "PATH", "path": "source.py", "sha256": "0" * 64}))
