...
def runtime_credential_fields(credential: str = "test-token") -> dict[str, str]:
    """Return the same encrypted credential columns produced by Runtime Target registration."""

    ciphertext = Fernet(TEST_RUNTIME_CREDENTIAL_KEY.encode()).encrypt(credential.encode()).decode()
    return {
